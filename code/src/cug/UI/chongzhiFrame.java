...
public class chongzhiFrame extends JInternalFrame {
	...
	private JTextField rechargejtextfield;
	...
	public chongzhiFrame() {
		...
		JLabel rechargejlabel = new JLabel();	//创建充值标签
		rechargejlabel.setText("充值：");			//设置标签文本
		Panel1.add(rechargejlabel);				//添加到面板1
		rechargejtextfield = new JTextField();//创建重置金额文本框
		rechargejtextfield.setColumns(10);//设置文本框长度
		Panel1.add(rechargejtextfield);//添加到面板1
	...
	// 添加充值按钮的事件监听器
	class addmanActionListener implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			int a2 = Integer.parseInt(rechargejtextfield.getText());
			if(a2>=0)
			{
				int a3 = a4 +a2;
	...
	// 添加余额按钮的事件监听器
	class yueActionListener implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			if(cardidjtextfield.getText().equals(""))
				namejtextfield.setText("");
			else {
