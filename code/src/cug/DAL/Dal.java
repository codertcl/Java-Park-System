...
public class Dal {
	...
	public static charger  searchcharge(String sql){
		charger charger1 = new charger();//计费标准对象
		ResultSet rs = Dal.executeQuery(sql);//执行查询
		try {
			while(rs.next()) {//如果查询到了
				charger1.setCardtype(rs.getString("cardtype"));//设置计费标准卡的类型
				charger1.setCharge(rs.getInt("charge"));//设置计费标准单价
				charger1.setStationtype(rs.getString("stationtype"));//设置计费标准车位类型
			}	
		} catch (SQLException e){
			e.printStackTrace();
		}
		Dal.close();	//关闭连接对象
		return charger1;//返回计费标准对象
	}
	...
	// 实现普通用户注册
	public static int comzhuce(int cardid,String name,String password,String cardtype,int overage,int tel,String carid){
		int i=0;
		try{
	...
	//车辆入场
	public static int parkin(String cardid, int stationid, String stationtype, String startpark, String carid){
		int i=0;
		try{
			String sql="insert park(cardid,stationid,stationtype,startpark,carid) values("+cardid+","+stationid+",'"+stationtype+"','"+startpark+"','"+carid+"')";//将入场车辆信息插入到数据库中
			i=Dal.executeUpdate(sql);
		}catch(Exception e){
			e.printStackTrace();
			System.out.print(1);
	...
	// 更改用户信息
	public static int updateuser(int cardid,String name,String password,String cardtype,int overage,int tel,String carid){
		int i=0;
		try{
